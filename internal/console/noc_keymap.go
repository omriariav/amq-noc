...
var nocKeyMap = []nocKeyBinding{
	...
	{Keys: []string{"m"}, HelpDisplay: "m", Label: "message the selected agent (kind selector); on an orchestrated squad's session or lead row: open the lead conversation", Footer: "m message", FooterAscii: "m message", Group: keyGroupAction, Mutating: true, Scopes: []nocNodeKind{nodeSession, nodeAgent}},
	...
	{Keys: []string{"L"}, HelpDisplay: "L", Label: "direct the lead of an orchestrated squad (pane when live, AMQ inbox when down; preview + confirm)", Footer: "L direct-lead", FooterAscii: "L direct-lead", Group: keyGroupAction, Mutating: true, Scopes: []nocNodeKind{nodeSession, nodeAgent}},
